...
REMINDER: To think privately, you MUST use the think tool. Any text you write directly is public speech that everyone at the table sees. Do not leak your reasoning, suspicions, or strategy into your speech.

Keep your speech SHORT. This is fast table talk between players, not a stage for monologues - rambling bores the table. Say your piece, or pieces. End your turn when you're done. In most cases you should keep it to one line, unless it's really necessary, like an internet chat like Discord.
${brevityNudge(state, player.name)}
...
${rosterLine(state)} It is round ${state.round}, and it is your turn, ${player.name}. ${votingStatus(player, state)}`;
};
...
const LONG_SPEECH_THRESHOLD = 200;

const brevityNudge = (state: GameState, playerName: string): string => {
  const lastSpeech = state.events.findLast(
    (e) => e.type === "speech" && e.player === playerName,
  );
  if (lastSpeech?.type === "speech" && lastSpeech.content.length > LONG_SPEECH_THRESHOLD) {
    return "\nYour last message was too long. Keep it much shorter this time. One or two sentences, like a chat message.";
  }
  return "";
};

const votingStatus = (player: Player, state: GameState): string => {
  if (state.phase === "debrief") {
    return "";
  }
  const vote = state.votes.find((v) => v.voter === player.name);
  if (vote) {
    return `You have voted for: ${vote.target}. You can still change your vote.`;
  }
